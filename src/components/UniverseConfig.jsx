...
export default function UniverseConfig({
  ...
  const recalcularUniversos = () => {
    const universesNeeded = Math.ceil(
      totalPixels / configuracoes.pixelsPerUniverse
    );
    ...
    const novosUniversos = Array.from({ length: universesNeeded }, (_, i) => ({
      ...
      endPixel: Math.min(
        (i + 1) * configuracoes.pixelsPerUniverse - 1,
        totalPixels - 1
      ),
      protocol: configuracoes.protocoloSaida,
      enabled: true,
      ip:
        configuracoes.protocoloSaida === "Art-Net" ? "2.0.0.1" : "239.255.0.1",
  ...
  const handleConfigChange = (field, value) => {
    const newConfig = {
      ...configuracoes,
      [field]:
        field === "protocoloSaida" || field === "rgbOrder"
          ? value
          : parseInt(value) || 1,
    };
    ...
    const updatedUniversos = universos.map((u) =>
      u.id === universeId
        ? {
            ...u,
            [field]:
              field === "protocol" || field === "ip"
                ? value
                : field === "enabled"
                ? value
                : parseInt(value) || u[field],
          }
        : u
    );
  ...
  const toggleUniverseEnabled = (universeId) => {
    handleUniverseChange(
      universeId,
      "enabled",
      !universos.find((u) => u.id === universeId)?.enabled
    );
  };
    ...
    switch (protocol) {
      case "Art-Net":
        return {
          ...
          defaultIP: "2.0.0.1",
          description:
            "Art-Net é o protocolo padrão para controle de iluminação profissional",
          maxChannels: 512,
          maxPixels: 170,
        };
      case "sACN":
        return {
          ...
          description:
            "sACN (E1.31) é um protocolo padrão ESTA para streaming de dados DMX",
          maxChannels: 512,
          maxPixels: 170,
        };
      case "DDP":
        return {
          ...
          description:
            "DDP é otimizado para pixels RGB, permitindo mais pixels por pacote",
          maxChannels: 1440,
          maxPixels: 480,
        };
      default:
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        ...
        >
          <div>
            <label
              style={{ color: "#b6c1e0", display: "block", marginBottom: 4 }}
            >
              Protocolo de Saída:
            </label>
            <select
              ...
              onChange={(e) =>
                handleConfigChange("protocoloSaida", e.target.value)
              }
              style={{
                width: "100%",
                padding: 8,
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                handleConfigChange("universoInicial", e.target.value)
              }
              style={{
                width: "100%",
                padding: 8,
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                handleConfigChange("pixelsPerUniverse", e.target.value)
              }
              style={{
                width: "100%",
                padding: 8,
            ...
            <div style={{ fontSize: "0.75em", color: "#9ca3af", marginTop: 4 }}>
              Máximo para {configuracoes.protocoloSaida}:{" "}
              {protocolInfo.maxPixels} pixels
            </div>
          </div>
        </div>
        ...
        >
          <strong>{configuracoes.protocoloSaida}:</strong>{" "}
          {protocolInfo.description}
          <br />
          Porta padrão: {protocolInfo.defaultPort} • IP padrão:{" "}
          {protocolInfo.defaultIP}
        </div>
      ...
      >
        <h4 style={{ color: "#fff", marginBottom: 12 }}>
          🌐 Universos DMX ({universos.length})
        </h4>
                ...
                >
                  ...
                  <div
                    style={{
                      ...
                      padding: 12,
                      cursor: "pointer",
                      background: universo.enabled
                        ? `${color}20`
                        : "transparent",
                    }}
                    onClick={() =>
                      setExpandedUniverse(isExpanded ? null : universo.id)
                    }
                  >
                    <div
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: "0.8em", color: "#b6c1e0" }}>
                          {pixelCount.toLocaleString()} pixels • {channelCount}{" "}
                          canais
                        </div>
                      </div>
                    </div>
                  ...
                  {isExpanded && (
                    <div style={{ padding: 16, paddingTop: 0 }}>
                      ...
                      >
                        <strong>Range de Pixels:</strong> {universo.startPixel}{" "}
                        - {universo.endPixel}
                        <br />
                        <strong>Total:</strong> {pixelCount} pixels (
                        {channelCount} canais DMX)
                      </div>
                    </div>
                  )}
                </div>
        ...
        {universos.length > 0 && (
          ...
          >
            ...
            <br />
            Total de universos: {universos.length}
            <br />
            Universos ativos: {universos.filter((u) => u.enabled).length}
            <br />
            Total de pixels: {totalPixels.toLocaleString()}
            <br />
