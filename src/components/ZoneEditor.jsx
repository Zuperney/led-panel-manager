...
export default function ZoneEditor({ 
  ...
  const getZoneUniverse = (zona) => {
    ...
    const universe = universos.find(u => 
      startPixel >= u.startPixel && startPixel <= u.endPixel
    );
    ...
    <div style={{ 
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {zonas.map((zona, index) => (
            <div
              key={zona.id}
              style={{
                background: selectedZone?.id === zona.id ? "#1f2937" : "#1a1d29",
                border: selectedZone?.id === zona.id ? "2px solid #3b82f6" : "1px solid #374151",
                borderRadius: 8,
                padding: 12,
                cursor: "pointer",
            ...
            >
                ...
                <div style={{ 
                  ...
                  <div
                    style={{
                      ...
                      height: 16,
                      borderRadius: 4,
                      background: ["#ef4444", "#f59e0b", "#10b981", "#3b82f6", "#8b5cf6", "#ec4899"][index % 6]
                    }}
                  />
                  {editingZone === zona.id ? (
                    <input
                      type="text"
                      value={zona.nome}
                      onChange={(e) => handleZoneChange(zona.id, 'nome', e.target.value)}
              ...
              <div style={{ 
                ...
                <div>Posição: {zona.x}, {zona.y}</div>
                <div>Tamanho: {zona.width}×{zona.height}</div>
              ...
              {selectedZone?.id === zona.id && (
                <div style={{ 
                  ...
                  <div>
                    <label style={{ color: "#9ca3af", fontSize: "0.75em", display: "block" }}>
                      X:
                    </label>
                    <input
                      type="number"
                      min="0"
                      value={zona.x}
                      onChange={(e) => handleZoneChange(zona.id, 'x', e.target.value)}
                      style={{
                        width: "100%",
                        padding: "4px 6px",
                        borderRadius: 4,
                  ...
                  <div>
                    ...
                    <input
                      ...
                      min="0"
                      value={zona.y}
                      onChange={(e) => handleZoneChange(zona.id, 'y', e.target.value)}
                      style={{
                        width: "100%",
                        padding: "4px 6px",
                        borderRadius: 4,
                  ...
                  <div>
                    ...
                    <input
                      ...
                      min="1"
                      value={zona.width}
                      onChange={(e) => handleZoneChange(zona.id, 'width', e.target.value)}
                      style={{
                        width: "100%",
                        padding: "4px 6px",
                        borderRadius: 4,
                  ...
                  <div>
                    ...
                    <input
                      ...
                      min="1"
                      value={zona.height}
                      onChange={(e) => handleZoneChange(zona.id, 'height', e.target.value)}
                      style={{
                        width: "100%",
                        padding: "4px 6px",
                        borderRadius: 4,
                  ...
                  <div style={{ gridColumn: "1 / -1" }}>
                    ...
                    <input
                      ...
                      onChange={(e) => handleZoneChange(zona.id, 'pixelStart', e.target.value)}
                      style={{
                        width: "100%",
                        padding: "4px 6px",
                        borderRadius: 4,
      ...
      {zonas.length > 0 && (
        <div style={{
          ...
          Total de zonas: {zonas.length}<br/>
          Total de pixels mapeados: {zonas.reduce((sum, zona) => sum + getZonePixelCount(zona), 0).toLocaleString()}
        </div>
      )}
    </div>
  );
