...
export default function ResolumeImporter({
  ...
  const parseResolumeXML = useCallback(
    (xmlText) => {
      try {
        ...
        const versionInfo = xmlDoc.querySelector(
          "versionInfo[name='Resolume Arena']"
        );
        if (!versionInfo) {
        ...
        const canvasSize = xmlDoc.querySelector(
          "CurrentCompositionTextureSize"
        );
        const canvasWidth = parseInt(canvasSize?.getAttribute("width")) || 1920;
        const canvasHeight =
          parseInt(canvasSize?.getAttribute("height")) || 1080;
        ...
        slices.forEach((slice, index) => {
          const name =
            slice.querySelector("Param[name='Name']")?.getAttribute("value") ||
            `Painel ${index + 1}`;
          ...
          if (vertices && vertices.length >= 4) {
            ...
            let matchedPanel = availablePanels?.find(
              (p) =>
        ...
        return {
          projectName:
            xmlDoc.querySelector("XmlState")?.getAttribute("name") ||
            "Layout Importado",
          canvasSize: { width: canvasWidth, height: canvasHeight },
          paineis: paineis,
          screenName:
            xmlDoc.querySelector("Screen")?.getAttribute("name") || "Screen 1",
        };
      } catch (error) {
  ...
  const handleFileUpload = useCallback(
    async (event) => {
      ...
      try {
        ...
        if (onFeedback) {
          onFeedback(
            `📥 XML analisado: ${parsedData.paineis.length} painéis encontrados!`
          );
        }
      } catch (error) {
        if (onFeedback) {
    ...
    >
      ...
      {!importData ? (
        <div>
          ...
          >
            Importe um arquivo XML de pixel mapping do Resolume Arena para
            recriar o layout de painéis.
          </p>
        ...
        <div>
          ...
          >
            ...
            <div style={{ marginBottom: 8 }}>
              <strong>📐 Canvas:</strong> {importData.canvasSize.width}×
              {importData.canvasSize.height}px
            </div>
            <div style={{ marginBottom: 8 }}>
              <strong>🖥️ Tela:</strong> {importData.screenName}
            </div>
            <div>
              <strong>📦 Painéis:</strong> {importData.paineis.length} slices
              encontrados
            </div>
          </div>
          ...
          >
            {importData.paineis.map((painel, index) => (
              ...
              >
                ...
                <div>
                  {painel.pixelsWidth}×{painel.pixelsHeight}px | Pos:{" "}
                  {Math.round(painel.x)}, {Math.round(painel.y)}
                </div>
              </div>
            ))}
          </div>
