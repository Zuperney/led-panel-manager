...
export function usePainelCrud({
   ...
   */
  const validarDuplicidade = useCallback((nome, isEdicao = false) => {
  ...
  const criarPainel = useCallback(async (dadosPainel) => {
    try {
      ...
      if (validarDuplicidade(dadosPainel.nome)) {
        alert('Já existe um painel com esse nome neste projeto. Escolha outro nome.');
        return false;
      }
  ...
  }, [paineis, setPaineis, selectedProjectId, salvarPaineis, showFeedback, validarDuplicidade]);
   ...
   */
  const editarPainel = useCallback(async (index, dadosPainel) => {
    try {
      const painelEditado = paineisFiltrados[index];
      
      // Encontra o índice real no array completo
      const idxReal = paineis.findIndex(p =>
      ...
      showFeedback(`Painel "${painelAtualizado.nome}" atualizado com sucesso!`);
      return true;
    } catch (error) {
  ...
  }, [paineis, paineisFiltrados, setPaineis, selectedProjectId, salvarPaineis, showFeedback]);
   ...
   */
  const removerPainel = useCallback(async (index) => {
    try {
   ...
   */
  const duplicarPainel = useCallback(async (index) => {
    try {
   ...
   */
  const iniciarEdicao = useCallback((index) => {
