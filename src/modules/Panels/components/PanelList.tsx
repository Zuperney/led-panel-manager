...
const PanelList: React.FC<PanelListProps> = ({
  ...
  const filteredPanels = panels.filter((panel) => {
    ...
    if (
      ...
      panel.manufacturer !== selectedFilters.manufacturer
    ) {
      return false;
    }

    if (
      selectedFilters.ipRating &&
      panel.ipRating !== selectedFilters.ipRating
    ) {
      return false;
    }
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            ...
            <p className="text-gray-600 mb-6">
              ...
              <strong>{deleteConfirm.panel.name}</strong>? Esta ação não pode
              ser desfeita.
            </p>
