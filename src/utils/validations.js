...
export const validations = {
  ...
  validateGabinete: (gabinete) => {
    ...
    if (!gabinete.largura || gabinete.largura < APP_CONFIG.VALIDACAO.MIN_LARGURA) {
    ...
    if (!gabinete.pixels_largura || gabinete.pixels_largura < APP_CONFIG.VALIDACAO.MIN_PIXELS) {
      errors.pixels_largura = 'Pixels em largura deve ser maior que 0';
    }
    
    if (!gabinete.pixels_altura || gabinete.pixels_altura < APP_CONFIG.VALIDACAO.MIN_PIXELS) {
      errors.pixels_altura = 'Pixels em altura deve ser maior que 0';
    }
    
    if (!gabinete.potencia || gabinete.potencia < APP_CONFIG.VALIDACAO.MIN_POTENCIA) {
  ...
  validatePainel: (painel) => {
    ...
    if (painel.modo === 'gabinete') {
      if (!painel.qtdLargura || painel.qtdLargura < 1) {
        errors.qtdLargura = 'Quantidade de gabinetes em largura deve ser maior que 0';
      }
      if (!painel.qtdAltura || painel.qtdAltura < 1) {
        errors.qtdAltura = 'Quantidade de gabinetes em altura deve ser maior que 0';
      }
    } else if (painel.modo === 'metro') {
      if (!painel.larguraM || painel.larguraM < APP_CONFIG.VALIDACAO.MIN_LARGURA) {
